...
from periocular_utils import get_face_landmarks, is_occluded, vectorize_landmarks, get_periocular_landmarks
from sklearn.metrics.pairwise import cosine_similarity

# --- Configuration & Efficiency Tuning ---
TARGET_CONFIDENCE = 0.95
PERIOCULAR_CONFIDENCE_THRESHOLD = 0.92 # <-- ADD: Similarity threshold for periocular match
...
print("[INFO] Loading aggregated periocular data...")
periocularDataFile = "output/periocular_data.pickle"
try:
    periocular_data = pickle.loads(open(periocularDataFile, "rb").read())
    print(f"[INFO] Loaded periocular vectors for {len(periocular_data)} individuals.")
except FileNotFoundError:
    print("[WARNING] periocular_data.pickle not found. Occlusion detection will be disabled.")
    periocular_data = None
...
while (time.time() - start_time) < MAX_RUN_TIME_SECONDS and not attendance_marked:
    ...
    # --- POSITIONAL GUIDANCE ---
    # Draw guide box and check if the user is in the correct position
    ...
    # Process only the largest face if one was found and positioned well
    if rects and is_positioned_well:
        ...
        # --- OCCLUSION DETECTION & DUAL PATHWAY RECOGNITION ---
        live_landmarks = get_face_landmarks(face)
        occluded = is_occluded(live_landmarks, face.shape) if live_landmarks and periocular_data else False
        print(f"DEBUG: Occlusion detected? {occluded}")
        # --- PATHWAY 1: PERIOCULAR (IF OCCLUDED) ---
        if occluded:
            live_vector = vectorize_landmarks(get_periocular_landmarks(live_landmarks, face.shape))

            if live_vector is not None:
                best_match_name = ""
                best_avg_similarity = 0.0

                # Compare against all known periocular vectors
                for name, stored_vectors in periocular_data.items():
                    # Calculate cosine similarity between live vector and all stored vectors for the person
                    sims = cosine_similarity(live_vector.reshape(1, -1), stored_vectors)
                    avg_sim = np.mean(sims)  # Use mean similarity for robustness
                ...
                print(f"DEBUG: Best periocular match for '{best_match_name}' with similarity: {best_avg_similarity:.4f}")
                if best_avg_similarity >= PERIOCULAR_CONFIDENCE_THRESHOLD:
                    attendance_marked = True
                    roll_number = student_data.get(best_match_name, "N/A")
                    recognized_person = (best_match_name, roll_number, best_avg_similarity)
                    display_text = f"{best_match_name}: {best_avg_similarity * 100:.2f}% (Periocular)"
                    display_color = (0, 255, 0)  # Green for success
                else:
                    display_text = f"Occluded: No Match ({best_avg_similarity * 100:.1f}%)"
                    display_color = (0, 0, 255)  # Red for fail
            else:
                display_text = "Occluded: Landmark Error"
                display_color = (0, 0, 255)

        # --- PATHWAY 2: FULL FACE (IF NOT OCCLUDED) ---
        else:
            # This is your original, existing logic
            ...
            # --- THIS IS THE NEW, STRICTER LOGIC WITH "UNKNOWN" ---
            if proba >= TARGET_CONFIDENCE:
                ...
                display_text = f"{name}: {proba * 100:.2f}%"
                display_color = (0, 255, 0)  # Green for success
            else:
                # The match is not confident enough, label as Unknown
                display_text = f"Unknown: ({proba * 100:.1f}%)"
                display_color = (0, 0, 255)  # Red for unknown

        # Draw box and text for both pathways
